...
evenness8 = read_qza("Results/day8-core-metrics-results/evenness_vector.qza")
evenness8<-evenness8$data %>% rownames_to_column("SampleID") # this moves the sample names to a new column that matches the metadata and allows them to be merged

observed_features8 = read_qza("Results/day8-core-metrics-results/observed_features_vector.qza")
observed_features8<-observed_features8$data %>% rownames_to_column("SampleID") # this moves the sample names to a new column that matches the metadata and allows them to be merged

shannon8 = read_qza("Results/day8-core-metrics-results/shannon_vector.qza")
shannon8<-shannon8$data %>% rownames_to_column("SampleID") # this moves the sample names to a new column that matches the metadata and allows them to be merged

faith_pd = read_qza("Results/day8-core-metrics-results/faith_pd_vector.qza")
faith_pd8<-faith_pd$data %>% rownames_to_column("SampleID") # this moves the sample names to a new column that matches the metadata and allows them to be merged\
faith_pd8<-faith_pd8[,-1]
colnames(faith_pd8)[1]<- "SampleID"
colnames(faith_pd8)[2]<- "Faith_PD"
...
alpha_diversity8 = merge(x=faith_pd8, y=evenness8, by.x = "SampleID", by.y = "SampleID")
alpha_diversity8 = merge(alpha_diversity8, observed_features8, by.x = "SampleID", by.y = "SampleID")
alpha_diversity8 = merge(alpha_diversity8, shannon8, by.x = "SampleID", by.y = "SampleID")
metadata8 = merge(metadata, alpha_diversity8, by.x = "SampleID", by.y = "SampleID")
...
#Plots
hist(metadata8$shannon_entropy, main="Shannon Diversity Day 8", xlab="", breaks=18)
hist(metadata8$Faith_PD, main="Faith phylogenetic Diversity Day 8", xlab="", breaks=18)
hist(metadata8$pielou_evenness, main="Evenness Day 8", xlab="", breaks=18)
hist(as.numeric(metadata8$observed_features), main="Observed Features Day 8", xlab="", breaks=18)

#Plots the qq-plot for residuals
ggqqplot(metadata8$shannon_entropy, title = "Shannon D. 8")
ggqqplot(metadata8$Faith_PD, title = "Faith PD D. 8")
ggqqplot(metadata8$pielou_evenness, title = "Evenness D. 8")
ggqqplot(metadata8$observed_features, title = "Observed Features D. 8")
...
shapiro.test(metadata8$shannon_entropy)
shapiro.test(metadata8$Faith_PD)
shapiro.test(metadata8$pielou_evenness)
shapiro.test(metadata8$observed_features)
...
# the n is low, like less than 38.

# Our main variables of interest are

# body site: gut, tongue, right palm, left palm
# subject: 1 and 2
# month-year: 18-2888, 1-2889, 2-2889, 3-2889, 4-2889
...
aov.evenness.trt.d8 = aov(pielou_evenness ~ Treatment, data=metadata8)
#Call for the summary of that ANOVA, which will include P-values
summary(aov.evenness.trt.d8)

aov.faith_pd.trt.d8 = aov(Faith_PD ~ Treatment, data=metadata8)
#Call for the summary of that ANOVA, which will include P-values
summary(aov.faith_pd.trt.d8)
...
levels(metadata8$Treatment)
#Re-order the groups because the default is alphabetical order
metadata8$TreatmentD8.ord = factor(metadata8$Treatment, c("CON", "AGO"))
levels(metadata8$TreatmentD8.ord)

#Plot
boxplot(pielou_evenness ~ TreatmentD8.ord, data=metadata8, ylab="Peilou Evenness Day 8")

evenness_bp8 <- ggplot(metadata8, aes(Treatment, pielou_evenness, color=Treatment)) + 
  geom_boxplot() + 
  #facet_grid(~TreatmentD8.ord) +
  #ylim(c(8.5,1)) +
  theme_q2r() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5))
ggsave("output/evennessd8.png", evenness_bp8, height = 3, width = 3)
...
evenness_summary8 <- metadata8 %>% # the names of the new data frame and the data frame to be summarised
  group_by(TreatmentD8.ord) %>%   # the grouping variable
  summarise(mean_evenness8 = mean(pielou_evenness),  # calculates the mean of each group
            sd_evenness8 = sd(pielou_evenness), # calculates the standard deviation of each group
            n_evenness8 = n(),  # calculates the sample size per group
            se_evenness8 = sd(pielou_evenness)/sqrt(n())) # calculates the standard error of each group
...
evenness_se8 <- ggplot(evenness_summary8, aes(TreatmentD8.ord, mean_evenness8, fill = TreatmentD8.ord)) + 
  geom_col() + 
  geom_errorbar(aes(ymin = mean_evenness8 - se_evenness8, ymax = mean_evenness8 + se_evenness8), width=8.2) + 
  theme_q2r() +
  theme(axis.text.x = element_text(angle = 98, hjust = 1, vjust = 8.5)) +
  theme(legend.title = element_blank()) +
  labs(y="Pielou's evenness  ± s.e.", x = "") 

ggsave("output/evenness_se8.png", evenness_se8, height = 2.5, width = 3)

# or with ggplot2
boxplot(Faith_PD ~ TreatmentD8.ord, data=metadata8, ylab="Faith Phylogenetic Diversity Day 8")

faith_pd8_plot <- ggplot(metadata8, aes(TreatmentD8.ord, Faith_PD)) + 
  geom_boxplot(aes(color = Treatment)) + 
  #ylim(c(8.5,1)) +
  theme_q2r() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
  theme(legend.title = element_blank()) +
  labs(y="Faith Phylogenetic Diversity D. 8", x = "") 
ggsave("output/pd8.png", faith_pd8_plot, height = 2.5, width = 3)
...
glm.evenness.d8 = glm(pielou_evenness ~ TreatmentD8.ord, data=metadata)
summary(glm.evenness.d8)

#The output let's us know that the intercept of our model is significantly different from 8 but our slope (*e.g.* our variable of interest) is not. This makes sense when we look at the data.

plot(pielou_evenness ~ TreatmentD8.ord, data=metadata8)
#Add the glm best fit line
plot(pielou_evenness ~ TreatmentD8.ord, data=metadata8) + abline(glm.evenness.d8)
...
qp.faith.d8 = glm(Faith_PD ~ TreatmentD8.ord, data=metadata8, family="quasipoisson")
plot(qp.faith.d8, which=c(1,2))
...
# the axis with gaussian was from -5 to 18. So, we will use quasipoisson 
# and see that ADG does not to correlate to Chao richness.
summary(qp.faith.d8)

#Plot
plot(log(Faith_PD) ~ TreatmentD8.ord, data=metadata8, ylab="ln(Faith Phylo. Diversity D. 8)")
plot(log(Faith_PD) ~ TreatmentD8.ord, data=metadata8, ylab="ln(Faith Phylo. Diversity D. 8)") + abline(qp.faith.d8)
