...
physeqd14 <- qza_to_phyloseq(
  features="Results/day-14-core-metrics-results/rarefied_table.qza",
...
physeq_otu_tabled14 <- data.frame(otu_table(physeqd14), check.names = F)

tax.cleand14 = tax.clean[row.names(tax.clean) %in% rownames(physeq_otu_tabled14),]
metadata.filteredd14 = metadata[row.names(metadata) %in% colnames(physeq_otu_tabled14),]

#Assign as variables to be feed into phyloseq
OTU.physeqd14 = otu_table(as.matrix(physeq_otu_tabled14), taxa_are_rows=TRUE)

#our edited and formatted taxonomy table from the top of this script
tax.physeqd14 = tax_table(as.matrix(tax.cleand14))    
meta.physeqd14 = sample_data(metadata.filteredd14)

#We then merge these into an object of class phyloseq.

physeq_bar_plotd14 = phyloseq(OTU.physeqd14, tax.physeqd14, meta.physeqd14)
...
for(ml in my_level){
  ...
  taxa.summaryd14 <- physeq_bar_plot %>%
  ...
  taxa.summaryd14 <- as.data.frame(taxa.summaryd14)
  colnames(taxa.summaryd14)[1] <- my_column
  colnames(taxa.summaryd14)[2] <- ml
  
  physeq.taxa.maxd14 <- taxa.summaryd14 %>% 
    group_by(get(ml)) %>%
    summarise(overall.max=max(Abundance.average))
  
  physeq.taxa.maxd14 <- as.data.frame(physeq.taxa.maxd14)
  colnames(physeq.taxa.maxd14)[1] <- ml
  
  # merging the phyla means with the metadata #
  physeq_metad14 <- merge(taxa.summaryd14, physeq.taxa.maxd14)
  
  
  physeq_meta_filteredd14 <- filter(physeq_metad14, overall.max>abund_filter)
  #str(physeq_meta_filtered)
  
  physeq_meta_filteredd14$Treatment= factor(physeq_meta_filteredd14$Treatment, c("CON", "AGO"))
  
  # Plot 
  ggplot(physeq_meta_filteredd14, aes(x = get(my_column), y = Abundance.average, fill = get(ml))) + 
  ...
  ggsave(paste0("output/", ml, "d14BarPlot_", my_column, ".png"), height = 5, width = 4)
}
...
#First get the OTU table from physeq
physeq_otu_tabled14 <- data.frame(otu_table(physeqd14), check.names = FALSE)

OTU.cleand14 <- physeq_otu_tabled14 + 1


#Now make the phyloseq object:


OTU.physeqd14 = otu_table(as.matrix(OTU.cleand14), taxa_are_rows=TRUE)
tax.physeqd14 = tax_table(as.matrix(tax.cleand14))
meta.physeqd14 = sample_data(metadata.filteredd14)
...
physeq_deseqd14 = phyloseq(OTU.physeqd14, tax.physeqd14, meta.physeqd14)
...
diagddsd14 = phyloseq_to_deseq2(physeq_deseqd14, ~ Treatment)
diagddsd14 = DESeq(diagddsd14, test="Wald", fitType="parametric")
...
resd14 = results(diagddsd14, contrast = my_contrast, cooksCutoff = FALSE)

sigtabd14 = res[which(resd14$padj < alpha), ]
sigtab_testd14 <- as(sigtab, "data.frame")
sigtabd14 = cbind(as(sigtabd14, "data.frame"), as(tax_table(physeq_deseq)[rownames(sigtabd14), ], "matrix"))
#head(sigtab)


###Volcano Plot

with(resd14, plot(log2FoldChange, -log10(padj), pch=20, main="Volcano plot", xlim=c(-15,15)))

# Add colored points: blue if padj<0.01, red if log2FC>1 and padj<0.05)
with(subset(resd14, padj<.01 ), points(log2FoldChange, -log10(pvalue), pch=20, col="purple"))
with(subset(resd14, padj<.01 & abs(log2FoldChange)>2), points(log2FoldChange, -log10(pvalue), pch=20, col="green"))
...
theme_set(theme_bw())
scale_fill_discreted14 <- function(palname = "Set1", ...) {
...
# Genus order
x = tapply(sigtabd14$log2FoldChange, sigtabd14$Genus, function(x) max(x))
x = sort(x, TRUE)
sigtabd14$Genus = factor(as.character(sigtabd14$Genus), levels=names(x))
DESeq_figd14 = ggplot(sigtabd14, aes(x=Genus, y = log2FoldChange, color=Phylum)) + 
  geom_point(size=3) + 
...
ggsave(paste0("output/DESeq2-d14", my_contrast[2], "-", my_contrast[3], ".png"), DESeq_figd14, height = 5, width = 10)
