...
evenness16 = read_qza("Results/day16-core-metrics-results/evenness_vector.qza")
evenness16<-evenness16$data %>% rownames_to_column("SampleID") # this moves the sample names to a new column that matches the metadata and allows them to be merged

observed_features16 = read_qza("Results/day16-core-metrics-results/observed_features_vector.qza")
observed_features16<-observed_features16$data %>% rownames_to_column("SampleID") # this moves the sample names to a new column that matches the metadata and allows them to be merged

shannon16 = read_qza("Results/day16-core-metrics-results/shannon_vector.qza")
shannon16<-shannon16$data %>% rownames_to_column("SampleID") # this moves the sample names to a new column that matches the metadata and allows them to be merged

faith_pd = read_qza("Results/day16-core-metrics-results/faith_pd_vector.qza")
faith_pd16<-faith_pd$data %>% rownames_to_column("SampleID") # this moves the sample names to a new column that matches the metadata and allows them to be merged\
faith_pd16<-faith_pd16[,-1]
colnames(faith_pd16)[1]<- "SampleID"
colnames(faith_pd16)[2]<- "Faith_PD"
...
alpha_diversity16 = merge(x=faith_pd16, y=evenness16, by.x = "SampleID", by.y = "SampleID")
alpha_diversity16 = merge(alpha_diversity16, observed_features16, by.x = "SampleID", by.y = "SampleID")
alpha_diversity16 = merge(alpha_diversity16, shannon16, by.x = "SampleID", by.y = "SampleID")
metadata16 = merge(metadata, alpha_diversity16, by.x = "SampleID", by.y = "SampleID")
...
#Plots
hist(metadata16$shannon_entropy, main="Shannon Diversity Day 16", xlab="", breaks=116)
hist(metadata16$Faith_PD, main="Faith phylogenetic Diversity Day 16", xlab="", breaks=116)
hist(metadata16$pielou_evenness, main="Evenness Day 16", xlab="", breaks=116)
hist(as.numeric(metadata16$observed_features), main="Observed Features Day 16", xlab="", breaks=116)

#Plots the qq-plot for residuals
ggqqplot(metadata16$shannon_entropy, title = "Shannon D. 16")
ggqqplot(metadata16$Faith_PD, title = "Faith PD D. 16")
ggqqplot(metadata16$pielou_evenness, title = "Evenness D. 16")
ggqqplot(metadata16$observed_features, title = "Observed Features D. 16")
...
shapiro.test(metadata16$shannon_entropy)
shapiro.test(metadata16$Faith_PD)
shapiro.test(metadata16$pielou_evenness)
shapiro.test(metadata16$observed_features)
...
# the n is low, like less than 316.

# Our main variables of interest are

# body site: gut, tongue, right palm, left palm
# subject: 1 and 2
# month-year: 116-2161616, 1-216169, 2-216169, 3-216169, 4-216169
...
aov.evenness.trt.d16 = aov(pielou_evenness ~ Treatment, data=metadata16)
#Call for the summary of that ANOVA, which will include P-values
summary(aov.evenness.trt.d16)
...
levels(metadata16$Treatment)
#Re-order the groups because the default is alphabetical order
metadata16$TreatmentD16.ord = factor(metadata16$Treatment, c("CON", "AGO"))
levels(metadata16$TreatmentD16.ord)

#Plot
boxplot(pielou_evenness ~ TreatmentD16.ord, data=metadata16, ylab="Peilou Evenness Day 16")

evenness_bp16 <- ggplot(metadata16, aes(Treatment, pielou_evenness, color=Treatment)) + 
  geom_boxplot() + 
  #facet_grid(~TreatmentD16.ord) +
  #ylim(c(16.5,1)) +
  theme_q2r() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5))
ggsave("output/evennessd16.png", evenness_bp16, height = 3, width = 3)
...
evenness_summary16 <- metadata16 %>% # the names of the new data frame and the data frame to be summarised
  group_by(TreatmentD16.ord) %>%   # the grouping variable
  summarise(mean_evenness16 = mean(pielou_evenness),  # calculates the mean of each group
            sd_evenness16 = sd(pielou_evenness), # calculates the standard deviation of each group
            n_evenness16 = n(),  # calculates the sample size per group
            se_evenness16 = sd(pielou_evenness)/sqrt(n())) # calculates the standard error of each group
...
evenness_se16 <- ggplot(evenness_summary16, aes(TreatmentD16.ord, mean_evenness16, fill = TreatmentD16.ord)) + 
  geom_col() + 
  geom_errorbar(aes(ymin = mean_evenness16 - se_evenness16, ymax = mean_evenness16 + se_evenness16), width=16.2) + 
  theme_q2r() +
  theme(axis.text.x = element_text(angle = 916, hjust = 1, vjust = 16.5)) +
  theme(legend.title = element_blank()) +
  labs(y="Pielou's evenness  ± s.e.", x = "") 

ggsave("output/evenness_se16.png", evenness_se16, height = 2.5, width = 3)

# or with ggplot2
boxplot(Faith_PD ~ TreatmentD16.ord, data=metadata16, ylab="Faith Phylogenetic Diversity Day 16")

faith_pd16_plot <- ggplot(metadata16, aes(TreatmentD16.ord, Faith_PD)) + 
  geom_boxplot(aes(color = Treatment)) + 
  #ylim(c(16.5,1)) +
  theme_q2r() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
  theme(legend.title = element_blank()) +
  labs(y="Faith Phylogenetic Diversity D. 16", x = "") 
ggsave("output/pd16.png", faith_pd16_plot, height = 2.5, width = 3)
...
glm.evenness.d16 = glm(pielou_evenness ~ TreatmentD16.ord, data=metadata16)
summary(glm.evenness.d16)

#The output let's us know that the intercept of our model is significantly different from 16 but our slope (*e.g.* our variable of interest) is not. This makes sense when we look at the data.

plot(pielou_evenness ~ TreatmentD16.ord, data=metadata16)
#Add the glm best fit line
plot(pielou_evenness ~ TreatmentD16.ord, data=metadata16) + abline(glm.evenness.d16)
...
qp.faith.d16 = glm(Faith_PD ~ TreatmentD16.ord, data=metadata16, family="quasipoisson")
plot(qp.faith.d16, which=c(1,2))
...
# the axis with gaussian was from -5 to 116. So, we will use quasipoisson 
# and see that ADG does not to correlate to Chao richness.
summary(qp.faith.d16)

#Plot
plot(log(Faith_PD) ~ TreatmentD16.ord, data=metadata16, ylab="ln(Faith Phylo. Diversity D. 16)")
plot(log(Faith_PD) ~ TreatmentD16.ord, data=metadata16, ylab="ln(Faith Phylo. Diversity D. 16)") + abline(qp.faith.d16)
