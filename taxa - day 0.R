...
physeqd0 <- qza_to_phyloseq(
  features="Results/day0-core-metrics-results/rarefied_table.qza",
...
physeq_otu_tabled0 <- data.frame(otu_table(physeqd0), check.names = F)

tax.cleand0 = tax.clean[row.names(tax.clean) %in% rownames(physeq_otu_tabled0),]
metadata.filteredd0 = metadata[row.names(metadata) %in% colnames(physeq_otu_tabled0),]

#Assign as variables to be feed into phyloseq
OTU.physeqd0 = otu_table(as.matrix(physeq_otu_tabled0), taxa_are_rows=TRUE)

#our edited and formatted taxonomy table from the top of this script
tax.physeqd0 = tax_table(as.matrix(tax.cleand0))    
meta.physeqd0 = sample_data(metadata.filteredd0)

#We then merge these into an object of class phyloseq.

physeq_bar_plotd0 = phyloseq(OTU.physeqd0, tax.physeqd0, meta.physeqd0)
...
for(ml in my_level){
  ...
  taxa.summaryd0 <- physeq_bar_plotd0 %>%
  ...
  taxa.summaryd0 <- as.data.frame(taxa.summaryd0)
  colnames(taxa.summaryd0)[1] <- my_column
  colnames(taxa.summaryd0)[2] <- ml
  
  physeq.taxa.maxd0 <- taxa.summaryd0 %>% 
    group_by(get(ml)) %>%
    summarise(overall.max=max(Abundance.average))
  
  physeq.taxa.maxd0 <- as.data.frame(physeq.taxa.maxd0)
  colnames(physeq.taxa.maxd0)[1] <- ml
  
  # merging the phyla means with the metadata #
  physeq_metad0 <- merge(taxa.summaryd0, physeq.taxa.maxd0)
  
  
  physeq_meta_filteredd0 <- filter(physeq_metad0, overall.max>abund_filter)
  #str(physeq_meta_filtered)
  
  physeq_meta_filteredd0$Treatment= factor(physeq_meta_filteredd0$Treatment, c("CON", "AGO"))
  
  # Plot 
  ggplot(physeq_meta_filteredd0, aes(x = get(my_column), y = Abundance.average, fill = get(ml))) + 
  ...
  ggsave(paste0("output/", ml, "d0BarPlot_", my_column, ".png"), height = 8, width = 5)
}
...
#First get the OTU table from physeq
physeq_otu_tabled0 <- data.frame(otu_table(physeqd0), check.names = FALSE)

OTU.cleand0 <- physeq_otu_tabled0 + 1


#Now make the phyloseq object:


OTU.physeqd0 = otu_table(as.matrix(OTU.cleand0), taxa_are_rows=TRUE)
tax.physeqd0 = tax_table(as.matrix(tax.cleand0))
meta.physeqd0 = sample_data(metadata.filteredd0)
...
diagddsd0 = phyloseq_to_deseq2(physeq_deseqd0, ~ Treatment)
diagddsd0 = DESeq(diagddsd0, test="Wald", fitType="parametric")
...
resd0 = results(diagddsd0, contrast = my_contrast, cooksCutoff = FALSE)

sigtabd0 = res[which(resd0$padj < alpha), ]
sigtab_testd0 <- as(sigtab, "data.frame")
sigtabd0 = cbind(as(sigtabd0, "data.frame"), as(tax_table(physeq_deseq)[rownames(sigtabd0), ], "matrix"))
#head(sigtab)


###Volcano Plot

with(resd0, plot(log2FoldChange, -log10(padj), pch=20, main="Volcano plot", xlim=c(-15,15)))
...
theme_set(theme_bw())
scale_fill_discreted0 <- function(palname = "Set1", ...) {
...
# Genus order
x = tapply(sigtabd0$log2FoldChange, sigtabd0$Genus, function(x) max(x))
x = sort(x, TRUE)
sigtabd0$Genus = factor(as.character(sigtabd0$Genus), levels=names(x))
DESeq_figd0 = ggplot(sigtabd0, aes(x=Genus, y = log2FoldChange, color=Phylum)) + 
  geom_point(size=3) + 
...
ggsave(paste0("output/DESeq2-d0", my_contrast[2], "-", my_contrast[3], ".png"), DESeq_figd0, height = 5, width = 10)
