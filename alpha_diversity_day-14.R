...
evenness14 = read_qza("Results/day-14-core-metrics-results/evenness_vector.qza")
evenness14<-evenness14$data %>% rownames_to_column("SampleID") # this moves the sample names to a new column that matches the metadata and allows them to be merged

observed_features14 = read_qza("Results/day-14-core-metrics-results/observed_features_vector.qza")
observed_features14<-observed_features14$data %>% rownames_to_column("SampleID") # this moves the sample names to a new column that matches the metadata and allows them to be merged

shannon14 = read_qza("Results/day-14-core-metrics-results/shannon_vector.qza")
shannon14<-shannon14$data %>% rownames_to_column("SampleID") # this moves the sample names to a new column that matches the metadata and allows them to be merged

faith_pd = read_qza("Results/day-14-core-metrics-results/faith_pd_vector.qza")
faith_pd14<-faith_pd$data %>% rownames_to_column("SampleID") # this moves the sample names to a new column that matches the metadata and allows them to be merged\
faith_pd14<-faith_pd14[,-1]
colnames(faith_pd14)[1]<- "SampleID"
colnames(faith_pd14)[2]<- "Faith_PD"
...
alpha_diversity14 = merge(x=faith_pd14, y=evenness14, by.x = "SampleID", by.y = "SampleID")
alpha_diversity14 = merge(alpha_diversity14, observed_features14, by.x = "SampleID", by.y = "SampleID")
alpha_diversity14 = merge(alpha_diversity14, shannon14, by.x = "SampleID", by.y = "SampleID")
metadata14 = merge(metadata, alpha_diversity14, by.x = "SampleID", by.y = "SampleID")
...
#Plots
hist(metadata14$shannon_entropy, main="Shannon Diversity Day -14", xlab="", breaks=10)
hist(metadata14$Faith_PD, main="Faith phylogenetic Diversity Day -14", xlab="", breaks=10)
hist(metadata14$pielou_evenness, main="Evenness Day -14", xlab="", breaks=10)
hist(as.numeric(metadata14$observed_features), main="Observed Features Day -14", xlab="", breaks=10)

#Plots the qq-plot for residuals
ggqqplot(metadata14$shannon_entropy, title = "Shannon D. -14")
ggqqplot(metadata14$Faith_PD, title = "Faith PD D. -14")
ggqqplot(metadata14$pielou_evenness, title = "Evenness D. -14")
ggqqplot(metadata14$observed_features, title = "Observed Features D. -14")
...
shapiro.test(metadata14$shannon_entropy)
shapiro.test(metadata14$Faith_PD)
shapiro.test(metadata14$pielou_evenness)
shapiro.test(metadata14$observed_features)
...
aov.evenness.trt.d14 = aov(pielou_evenness ~ Treatment, data=metadata14)
#Call for the summary of that ANOVA, which will include P-values
summary(aov.evenness.trt.d14)
...
levels(metadata14$Treatment)
#Re-order the groups because the default is alphabetical order
metadata14$TreatmentD14.ord = factor(metadata14$Treatment, c("CON", "AGO"))
levels(metadata14$TreatmentD14.ord)

#Plot
boxplot(pielou_evenness ~ TreatmentD14.ord, data=metadata14, ylab="Peilou Evenness Day -14")

evenness_bp14 <- ggplot(metadata14, aes(Treatment, pielou_evenness, color=Treatment)) + 
  geom_boxplot() + 
  #facet_grid(~TreatmentD14.ord) +
  #ylim(c(0.5,1)) +
  theme_q2r() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5))
ggsave("output/evennessd14.png", evenness_bp14, height = 3, width = 3)
...
evenness_summary14 <- metadata14 %>% # the names of the new data frame and the data frame to be summarised
  group_by(TreatmentD14.ord) %>%   # the grouping variable
  summarise(mean_evenness14 = mean(pielou_evenness),  # calculates the mean of each group
            sd_evenness14 = sd(pielou_evenness), # calculates the standard deviation of each group
            n_evenness14 = n(),  # calculates the sample size per group
            se_evenness14 = sd(pielou_evenness)/sqrt(n())) # calculates the standard error of each group
...
evenness_se14 <- ggplot(evenness_summary14, aes(TreatmentD14.ord, mean_evenness14, fill = TreatmentD14.ord)) + 
  geom_col() + 
  geom_errorbar(aes(ymin = mean_evenness14 - se_evenness14, ymax = mean_evenness14 + se_evenness14), width=0.2) + 
  theme_q2r() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
  theme(legend.title = element_blank()) +
  labs(y="Pielou's evenness  ± s.e.", x = "") 

ggsave("output/evenness_se14.png", evenness_se14, height = 2.5, width = 3)

# or with ggplot2
boxplot(Faith_PD ~ TreatmentD14.ord, data=metadata14, ylab="Faith Phylogenetic Diversity Day -14")

faith_pd14_plot <- ggplot(metadata14, aes(TreatmentD14.ord, Faith_PD)) + 
  geom_boxplot(aes(color = Treatment)) + 
  #ylim(c(0.5,1)) +
  theme_q2r() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
  theme(legend.title = element_blank()) +
  labs(y="Faith Phylogenetic Diversity D. -14", x = "") 
ggsave("output/pd14.png", faith_pd14_plot, height = 2.5, width = 3)
...
glm.evenness.d14 = glm(pielou_evenness ~ TreatmentD14.ord, data=metadata14)
summary(glm.evenness.d14)
...
plot(pielou_evenness ~ TreatmentD14.ord, data=metadata14)
#Add the glm best fit line
plot(pielou_evenness ~ TreatmentD14.ord, data=metadata14) + abline(glm.evenness.d14)
...
qp.faith.d14 = glm(Faith_PD ~ TreatmentD14.ord, data=metadata14, family="quasipoisson")
plot(qp.faith.d14, which=c(1,2))
...
summary(qp.faith.d14)

#Plot
plot(log(Faith_PD) ~ TreatmentD14.ord, data=metadata14, ylab="ln(Faith Phylo. Diversity D. -14)")
plot(log(Faith_PD) ~ TreatmentD14.ord, data=metadata14, ylab="ln(Faith Phylo. Diversity D. -14)") + abline(qp.faith.d14)
